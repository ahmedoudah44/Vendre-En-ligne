...
namespace WebApplication1.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        {
            ViewBag.UserType = new SelectList(db.Roles.Where(a => !a.Name.Contains("Admine")).ToList(), "Name", "Name");
            return View();
        }
        ...
        public ActionResult EditProfile(EditProfileViewModel profile)
        {
            ...
            if (!UserManager.CheckPassword(CurentUser, profile.CurrentPassword))
            {
                ViewBag.Message = "Cette Passworde n' est pas Valide";
            }
            else
            {
                var newPasswordHash = UserManager.PasswordHasher.HashPassword (profile.NewPassword);
                CurentUser.UserName = profile.UserName;
                CurentUser.Email =profile.Email;
                CurentUser.PasswordHash = newPasswordHash;
                db.Entry(CurentUser).State = System.Data.Entity.EntityState.Modified;
                db.SaveChanges();
                ViewBag.Message = "l'Opperation de Edite Est Resie";
            }
            return View(profile);
        }
